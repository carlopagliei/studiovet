...
namespace Carlop.Configuration
{
	...
	public abstract class XmlBasedCfgStorage : ConfigurationProviderBase
	{
		...
		protected XmlBasedCfgStorage(string fileName) :
			base(fileName)
		{
		}
		...
		protected XmlBasedCfgStorage(XmlBasedCfgStorage profile) :
			base(profile)
		{
		...
		public Encoding Encoding
		{
			...
			{ 
				...
				if (!RaiseChangeEvent(true, ConfigurationChangeType.Other, null, "Encoding", value))
					return;

				m_encoding = value; 				
				RaiseChangeEvent(false, ConfigurationChangeType.Other, null, "Encoding", value);				
			}
		}
	}
	...
	{
		private XmlBasedCfgStorage m_profile;
		...
		/// <exception cref="UnauthorizedAccessException">
		///	  Attempting to lock the file and ReadWrite access is not permitted by the operating system. </exception>
		internal XmlBuffer(XmlBasedCfgStorage profile, bool lockFile)
		{
			m_profile = profile;
