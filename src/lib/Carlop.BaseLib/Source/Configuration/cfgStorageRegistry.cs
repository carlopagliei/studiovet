...
namespace Carlop.Configuration
{
	...
	public class RegistryCfgStorage : ConfigurationProviderBase
	{
		// Fields
		...
		public RegistryCfgStorage(RegistryKey rootKey, string subKeyName) :
			base("")
		{
			if (rootKey != null)
				m_rootKey = rootKey;
		...
		public RegistryCfgStorage(RegistryCfgStorage reg) :
			base(reg)
		{
		...
		{
			return new RegistryCfgStorage(this);
		}
		...
		public RegistryKey RootKey
		{
			...
			{ 
				...
				if (!RaiseChangeEvent(true, ConfigurationChangeType.Other, null, "RootKey", value))
					return;
				
				m_rootKey = value; 
				RaiseChangeEvent(false, ConfigurationChangeType.Other, null, "RootKey", value);
			}
		}
      ...
      string keyName;
      if (section.ToLower().StartsWith(Name.ToLower())) { keyName = section; }
      else { keyName = Name + "\\" + section; }
