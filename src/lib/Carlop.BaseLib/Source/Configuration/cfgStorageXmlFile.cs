/*
 * AMS.Provider Class Library
 * 
 * Written by Alvaro Mendez
 * Copyright (c) 2004. All Rights Reserved.
 * 
 * The AMS.Provider namespace contains interfaces and classes that 
 * allow reading and writing of user-Provider data.
...
{
	/// <summary>
	///   Provider class that utilizes an XML file to retrieve and save its data. </summary>
	/// <remarks>
	...
	///   &lt;/Provider&gt;
	///   </code></remarks>
	public class XmlFileCfgStorage : XmlBasedCfgStorage
	{
		// Fields
		private string m_rootName = "Provider";

		/// <summary>
		///   Initializes a new instance of the Xml class by setting the <see cref="Provider.Name" /> to <see cref="Provider.DefaultName" />. </summary>
		public XmlFileCfgStorage()
		{
		}

		/// <summary>
		///   Initializes a new instance of the Xml class by setting the <see cref="Provider.Name" /> to the given file name. </summary>
		/// <param name="fileName">
		///   The name of the XML file to initialize the <see cref="Provider.Name" /> property with. </param>
		public XmlFileCfgStorage(string fileName) :
			base(fileName)
		{
		}
		...
		public XmlFileCfgStorage(XmlFileCfgStorage xml) :
			base(xml)
		{
		...
		///   This property is used to set the <see cref="Provider.Name" /> property inside the default constructor.</remarks>
		...
		/// <seealso cref="Provider.CloneReadOnly" />
		public override object Clone()
		{
			return new XmlFileCfgStorage(this);
		}
		...
		///   Setting this property if <see cref="Provider.ReadOnly" /> is true. </exception>
		/// <exception cref="NullReferenceException">
		///   Setting this property to null. </exception>
		/// <remarks>
		///   By default this property is set to "Provider", but it is only used when the file 
		...
		///   The <see cref="Provider.Changing" /> event is raised before changing this property.  
		///   If its <see cref="ProviderChangingArgs.Cancel" /> property is set to true, this method 
		///   returns immediately without changing this property.  After the property has been changed, 
		///   the <see cref="Provider.Changed" /> event is raised. </remarks>
		public string RootName
		{
			...
			{ 
				...
				if (!RaiseChangeEvent(true, ConfigurationChangeType.Other, null, "RootName", value))
					return;

				m_rootName = value.Trim(); 				
				RaiseChangeEvent(false, ConfigurationChangeType.Other, null, "RootName", value);				
			}
		}
		...
		/// <exception cref="InvalidOperationException">
		///   <see cref="Provider.Name" /> is null or empty, 
		///   <see cref="Provider.ReadOnly" /> is true, or
		...
		///   The <see cref="Provider.Changing" /> event is raised before setting the value.  
		///   If its <see cref="ProviderChangingArgs.Cancel" /> property is set to true, this method 
		///   returns immediately without setting the value.  After the value has been set, 
		///   the <see cref="Provider.Changed" /> event is raised. 
		///   <para>
		...
		/// <exception cref="InvalidOperationException">
		///	  <see cref="Provider.Name" /> is null or empty. </exception>
		/// <exception cref="ArgumentNullException">
		///   Either section or entry is null. </exception>
		/// <exception cref="XmlException">
		///	  Parse error in the XML being loaded from the file. </exception>
		/// <seealso cref="SetValue" />
		/// <seealso cref="Provider.HasEntry" />
		...
		/// <exception cref="InvalidOperationException">
		///	  <see cref="Provider.Name" /> is null or empty or
		///   <see cref="Provider.ReadOnly" /> is true. </exception>
		/// <exception cref="ArgumentNullException">
		///   Either section or entry is null. </exception>
		/// <exception cref="XmlException">
		///	  Parse error in the XML being loaded from the file or
		///	  the resulting XML document would not be well formed. </exception>
		/// <remarks>
		///   The <see cref="Provider.Changing" /> event is raised before removing the entry.  
		///   If its <see cref="ProviderChangingArgs.Cancel" /> property is set to true, this method 
		...
		///	  the resulting XML document would not be well formed. </exception>
		/// <remarks>
		///   The <see cref="Provider.Changing" /> event is raised before removing the section.  
		///   If its <see cref="ProviderChangingArgs.Cancel" /> property is set to true, this method 
		...
		/// <exception cref="XmlException">
		///	  Parse error in the XML being loaded from the file. </exception>
		/// <seealso cref="Provider.HasSection" />
